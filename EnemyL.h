...
public:
  ...
  : Enemy(pos, size, sprite, velocity, hitPoints, type) { this->Money = 45; }
  virtual void UpdateEnemy(GLfloat dt, GLfloat WidthLeft, GLfloat WidthRight, GLuint Width, GLuint Height, GLfloat Time) override
  {
    ...
    if (this->Velocity.y <= 120 && this->Position.x <= WidthRight - this->Size.x && this->Position.x >= Width / 2)
    {
      this->Velocity.x -= 100 * dt;
    }
    if (this->Velocity.y <= 120 && this->Position.x >= WidthLeft && this->Position.x <= Width / 2)
    {
      this->Velocity.x += 100 * dt;
    }
