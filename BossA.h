...
public:
	BossA()
		: Enemy() {}
	BossA(glm::vec2 pos, glm::vec2 size, Texture2D sprite, glm::vec2 velocity, GLfloat hitPoints, GLint type)
		: Enemy(pos, size, sprite, velocity, hitPoints, type), Move(0), SwitchMove(GL_FALSE), PeriodBuf(2),LastMove(3), MoveRight(GL_TRUE), MoveLeft(GL_FALSE), MoveUp(GL_TRUE), MoveDown(GL_FALSE), ActiveMove(GL_FALSE), BulletActive(GL_FALSE), Shoot(GL_FALSE), NumberShoot(0) { this->Boss = GL_TRUE; this->Money = 600; }
	virtual void UpdateEnemy(GLfloat dt, GLfloat WidthLeft, GLfloat WidthRight, GLuint Width, GLuint Height, GLfloat Time) override
	{
		...
		if (this->Move == 1)
		{
			...
			if (this->PeriodBuf > 0)
			{
				if (this->Position.x <= WidthRight - this->Size.x && this->Position.x >= Width - Width / 3)
				{
					this->MoveLeft = GL_TRUE;
					this->MoveRight = GL_FALSE;
				}
				if (this->Position.x >= WidthLeft && this->Position.x <= Width / 3)
				{
		...
		if (this->Move == 2)
		{
			...
			if (this->PeriodBuf > 0)
			{
				...
				if (this->Position.y <= Height / 5)
				{
					...
					if (this->Position.x >= WidthRight - this->Size.x * 2)
					{
						this->Velocity.x = -1;
					}
		...
		if (this->Move == 3)
		{
			...
			if (this->PeriodBuf > 0)
			{
				if (this->Position.y <= Height / 3 && this->Position.y >= 0)
				{
					this->MoveDown = GL_TRUE;
					this->MoveUp = GL_FALSE;
				}
				if (this->Position.y >= Height - Height / 3 - this->Size.y && this->Position.y <= Height - this->Size.y)
				{
					this->MoveUp = GL_TRUE;
	...
	virtual void SpawnBullets(GLfloat Time, GLfloat dt, GLfloat PlayerPosx, GLfloat PlayerPosy, std::vector<Bullet>* Bullets) override
	{
		...
		if (this->Move == 2)
		{
			...
			if (Time / this->PeriodBullet >= 1 && this->OnScreen)
			{
				...
				if (this->NumberShoot < 16)
				{
					this->PeriodBullet += 4 * dt;
					this->NumberShoot += 1;
				}
				else
				{
					this->PeriodBullet += 200 * dt;
					this->NumberShoot = 0;
				}
			}
		}
